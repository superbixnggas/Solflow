...
const RebalanceView: React.FC = () => {
  ...
  const [generatedPlan, setGeneratedPlan] = useState(rebalancePlan);
  const [lastUpdate] = useState(new Date());

  // Calculate deviations and status
  const deviations = targetAllocations.map(target => {
    ...
    const deviation = currentPercentage - target.target;
    const absDeviation = Math.abs(deviation);
    
    let status = 'balanced';
    if (absDeviation <= target.threshold) {
      status = 'balanced'; // Green - within threshold
    } else if (absDeviation <= target.threshold * 2) {
      status = 'warning'; // Yellow - warning zone  
    } else {
      status = 'critical'; // Red - critical zone
    }
    ...
    return {
      ...
      status: status,
      currentValue: currentToken ? currentToken.balance * currentToken.price : 0
    };
  });

  const needsRebalance = deviations.some(d => d.status !== 'balanced');
  ...
  const handleGeneratePlan = () => {
    ...
    setTimeout(() => {
      ...
      setGeneratedPlan(rebalancePlan);
    }, 2000);
  };

  const handleExecute = () => {
    // Simulate execution
    alert('Rebalance plan akan dieksekusi dalam versi production');
  };
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                <span className="text-gray-400 text-sm">
                  {generatedPlan.length} swap diperlukan
                </span>
              </div>
            </div>
            ...
            <div className="overflow-x-auto mb-6">
              <table className="w-full">
                ...
                <tbody>
                  {generatedPlan.map((action, index) => (
                    <motion.tr
                      key={index}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-white/5 rounded-lg p-4">
                ...
                <p className="text-white font-semibold text-xl">{generatedPlan.length}</p>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <p className="text-gray-400 text-sm">Est. Total Fees</p>
                <p className="text-green-400 font-semibold text-xl">
                  {generatedPlan.reduce((sum, a) => sum + a.feeEstimate, 0).toFixed(4)} SOL
                </p>
              </div>
              <div className="bg-white/5 rounded-lg p-4">
                <p className="text-gray-400 text-sm">Max Price Impact</p>
                <p className="text-yellow-400 font-semibold text-xl">
                  {Math.max(...generatedPlan.map(a => a.estimatedPriceImpact)).toFixed(2)}%
                </p>
              </div>
            </div>
